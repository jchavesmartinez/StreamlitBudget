...
with st.expander("Ingresos"):
    ...
    if st.checkbox("Registrar ingreso"):
        ...
        if submitted:
            db.COLLECTION_DIARIO.insert_one({"_id": len(Diario)+2, "Motivo": motivo_option, "Tipo": 'Ingreso', "Monto": -monto_ingreso, "Fecha": fecha, "Escenario": '2. Actual', "Cuenta": cuenta_option, "Nota": nota_input})
...
with st.expander("Gastos"):
    ...
    if st.checkbox("Registrar gasto"):
        ...
        if submitted2:
            db.COLLECTION_DIARIO.insert_one({"_id": len(Diario)+2, "Motivo": motivo_option2, "Tipo": 'Gasto', "Monto": -monto_ingreso2, "Fecha": fecha, "Escenario": '2. Actual', "Cuenta": cuenta_option2, "Nota": nota_input2})
            st.success('This is a success message!!', icon="✅")
            st.experimental_rerun()
...
with st.expander("Metricas y resultados"):
    ...
    Gastos_faltantes=sum(DiarioCalculosA[(DiarioCalculosA.Monto >= 0)])

    tab1, tab2= st.tabs(["Metricas", "Resumen"])

    with tab1:
        col1, col2, col3, col4,col5 = st.columns([1,1,1,1,1])
        col1.metric("Temperature", Gastos_faltantes, "1.2 °F")
    ...
    with tab4:
        ...
        if cuenta_option2=='Tarjeta credito Jose':
            ...
            if st.checkbox("Pagar tarjeta Jose"):
                ...
                if submitted31:
                    db.COLLECTION_DIARIO.insert_one({"_id": len(Diario)+2, "Motivo": 'Tasa 0', "Tipo": 'Gasto', "Monto": monto_ingreso21, "Fecha": fecha, "Escenario": '2. Actual', "Cuenta": 'Tarjeta credito Jose', "Nota": 'Pago tarjeta de credito Jose'})
                    db.COLLECTION_DIARIO.insert_one({"_id": len(Diario)+3, "Motivo": 'Tasa 0', "Tipo": 'Gasto', "Monto": -monto_ingreso21, "Fecha": fecha, "Escenario": '2. Actual', "Cuenta": cuenta_option21, "Nota": 'Pago tarjeta de credito Jose'})
                    st.success('This is a success message!!', icon="✅")
                    st.experimental_rerun()
        ...
        if cuenta_option2=='Tarjeta credito Aline':
            ...
            if st.checkbox("Pagar tarjeta Aline"):
                ...
                if submitted4:
                    db.COLLECTION_DIARIO.insert_one({"_id": len(Diario)+2, "Motivo": 'Tasa 0', "Tipo": 'Gasto', "Monto": monto_ingreso2, "Fecha": fecha, "Escenario": '2. Actual', "Cuenta": 'Tarjeta credito Aline', "Nota": 'Pago tarjeta de credito Aline'})
                    db.COLLECTION_DIARIO.insert_one({"_id": len(Diario)+3, "Motivo": 'Tasa 0', "Tipo": 'Gasto', "Monto": -monto_ingreso2, "Fecha": fecha, "Escenario": '2. Actual', "Cuenta": cuenta_option2, "Nota": 'Pago tarjeta de credito Aline'})
                    st.success('This is a success message!!', icon="✅")
                    st.experimental_rerun()
